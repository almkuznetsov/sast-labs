...
### Настройка окружения
Для выполнения данной работы потребуется предоставленный кафедрой доступ к Svace, поэтому настройка Svace здесь не описывается.
Настройка кафедрой должна подразумевать выполнение указаний [документации](https://svace.pages.ispras.ru/svace-website/ru/docs) до того момента, пока не появится возможность запуска `svace build`, `svace analyzer`.
...
Самостоятельно с помощью ресурсов с данными об уязвимости CVE-2021-41459 подготовьте патч с исправлением ошибки, убедитесь, что PoC-файл больше не вызывает исключение, а также, что статический анализ не вызывает сработку ранее указанного детектора.
...
### Настройка окружения
Аналогично 3 части, для выполнения данной работы потребуется предоставленный кафедрой доступ к Svace.
